...
function loadOrCreateSecret(): string {
  ...
  console.log(`[auth] JWT secret generated → ${SECRET_FILE}`);
  return secret;
}
...
app.get('/api/stats', authenticateToken, (_req, res) => {
  ...
  const dailyActivity   = db.prepare(`
    ...
    FROM audit_logs WHERE timestamp >= date('now', '-7 days')
...
app.get('/api/vault', authenticateToken, (req, res) => {
  ...
  res.json(items.map(i => ({ ...i, password: isViewer ? '••••••••••••' : decrypt(i.password) })));
});
