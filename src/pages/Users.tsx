  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 bg-theme min-h-full animate-in fade-in duration-700">
      <header className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-theme tracking-tight">User Management</h1>
          <p className="text-theme-2 font-medium text-sm">Control system access and assign roles.</p>
        </div>
        {canManageUsers && (
                  ...
                  <tr key={u.id} className="text-sm hover:bg-surface-2 transition-all group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-theme text-base">{u.name}</span>
                          {u.id === currentUser?.id && (
                    ...
                    <td className="px-8 py-6 text-theme-2 font-medium">
                      <div className="flex items-center gap-2">
                        <Mail size={14} className="text-theme-3" />
                        {u.email}
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      {isEditingThis ? (
                        <div className="flex items-center gap-2">
                          <select
                            ...
                            className="p-2 bg-surface-2 border border-theme rounded-xl text-sm font-bold text-theme outline-none"
                          >
                    ...
                    <td className="px-8 py-6 text-theme-3 font-medium text-xs">
                    ...
                    <td className="px-8 py-6 text-right">
                      {canManageUsers && (
                        <button
                          disabled={u.id === currentUser?.id}
                          onClick={() => handleDelete(u.id)}
                          className="p-3 hover:bg-rose-50  rounded-2xl text-theme-3 hover:text-rose-500  transition-all disabled:opacity-20 disabled:cursor-not-allowed"
                        >
                          <Trash2 size={20} />
                        </button>
                      )}
          ...
          <div className="bg-surface rounded-[2.5rem] border border-theme w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden">
            <div className="p-6 md:p-10 space-y-8">
              <div className="flex items-center gap-4">
                ...
                <h2 className="text-2xl font-bold text-theme tracking-tight">Create New User</h2>
              </div>
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-theme-3 tracking-widest ml-1">Full Name</label>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-theme-3 tracking-widest ml-1">Email Address</label>
                  <input required type="email" placeholder="user@airforce.mil"
                    className="w-full p-4 bg-surface-2/50 rounded-2xl border border-transparent focus:border-command-blue/20  focus:ring-4 focus:ring-command-blue/5 outline-none transition-all font-medium text-theme"
                    value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-theme-3 tracking-widest ml-1">Role</label>
                  <select className="w-full p-4 bg-surface-2/50 rounded-2xl border border-transparent focus:border-command-blue/20  focus:ring-4 focus:ring-command-blue/5 outline-none transition-all font-medium text-theme appearance-none"
                    value={formData.role} onChange={e => setFormData({...formData, role: e.target.value})}>
                    <option>Administrator</option>
                    <option>Operator</option>
                    <option>Viewer</option>
                  </select>
                  <p className="text-[10px] text-theme-3 ml-1 font-medium">
