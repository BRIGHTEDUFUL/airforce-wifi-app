  ...
  return (
    <div className="p-8 space-y-8 bg-command-light dark:bg-command-dark-bg min-h-full animate-in fade-in duration-700">
      <header className="flex justify-between items-center">
        ...
        <button className="bg-white dark:bg-command-dark-card hover:bg-slate-50 dark:hover:bg-white/5 text-slate-700 dark:text-slate-300 px-6 py-2.5 rounded-xl border border-slate-200 dark:border-command-dark-border flex items-center gap-2 transition-all shadow-sm font-bold text-sm">
          <Download size={18} />
          Export CSV
        </button>
      </header>
      ...
      <div className="bg-white dark:bg-command-dark-card rounded-[2.5rem] border border-slate-100 dark:border-command-dark-border shadow-sm overflow-hidden">
        <div className="p-6 border-b border-slate-50 dark:border-command-dark-border flex flex-col sm:flex-row gap-6">
          <div className="relative flex-1 group">
            ...
            <input 
              ...
              className="w-full pl-12 pr-4 py-3 bg-slate-50 dark:bg-slate-900/50 border border-transparent rounded-2xl outline-none focus:border-command-blue/20 focus:bg-white dark:focus:bg-slate-900 focus:ring-4 focus:ring-command-blue/5 transition-all text-sm font-medium text-slate-700 dark:text-slate-200"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <button className="px-6 py-3 bg-slate-50 dark:bg-slate-900/50 border border-transparent rounded-2xl flex items-center gap-2 text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/5 transition-all">
            <Filter size={16} />
            All Modules
          </button>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/50 dark:bg-slate-900/50 text-[10px] uppercase tracking-[0.2em] font-black text-slate-400 dark:text-slate-500">
            ...
            <tbody className="divide-y divide-slate-50 dark:divide-command-dark-border">
              {filteredLogs.map((log) => (
                <tr key={log.id} className="text-sm hover:bg-slate-50/50 dark:hover:bg-white/5 transition-all group">
