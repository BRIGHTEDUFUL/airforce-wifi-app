...
const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab }) => {
  ...
  const handleNavClick = (id: string) => { setActiveTab(id); setMobileOpen(false); };
  ...
  const themeIcon = theme === 'dark'
    ? <Moon size={18} className="group-hover:-rotate-12 transition-transform duration-700" />
    : theme === 'light'
    ? <Sun size={18} className="group-hover:rotate-90 transition-transform duration-700" />
    : <Monitor size={18} className="group-hover:scale-110 transition-transform duration-300" />;

  const sidebarContent = (isMobile = false) => (
    <div className={cn(
      "flex flex-col h-full transition-all duration-[250ms] border-r border-theme relative z-20 shadow-2xl bg-surface text-theme-2",
      !isMobile && (collapsed ? "w-20" : "w-64"),
      isMobile && "w-72"
    )}>
      <div className="p-6 flex flex-col items-center gap-4 border-b border-theme bg-surface">
        ...
        {(!collapsed || isMobile) && (
          <div className="text-center">
            <h2 className="text-sm font-bold tracking-tight text-theme">{APP_NAME} Management</h2>
            <p className="text-[10px] font-bold text-theme-3 uppercase tracking-widest">Ghana Air Force HQ</p>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-6">
        {(!collapsed || isMobile) && (
          <div className="px-6 mb-4">
            <p className="text-[10px] font-black text-theme-3 uppercase tracking-[0.2em]">
            ...
            <button key={item.id} onClick={() => handleNavClick(item.id)}
              className={cn(
                ...
                activeTab === item.id
                  ...
                  : "hover:bg-surface-2 text-theme-2 hover:text-theme"
              )}>
              {activeTab === item.id && <div className="absolute inset-0 bg-gradient-to-r from-white/10 to-transparent" />}
              <item.icon size={20} className={cn(
                "relative z-10 transition-transform group-hover:scale-110",
                activeTab === item.id ? "text-white" : "text-theme-3 group-hover:text-theme-2"
              )} />
              ...
              {activeTab === item.id && (!collapsed || isMobile) && (
                <div className="ml-auto relative z-10"><div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" /></div>
              )}
            </button>
          ))}
        </nav>
      ...
      <div className="p-4 space-y-3 border-t border-theme bg-surface">
        {(!collapsed || isMobile) && (
          <p className="text-[10px] font-black text-theme-3 uppercase tracking-[0.2em] mb-4 px-2">System Controls</p>
        )}
        {(!collapsed || isMobile) && (
          <div className="rounded-2xl p-3 border border-theme mb-4 group transition-all cursor-pointer relative overflow-hidden bg-surface-2 hover:bg-surface">
            ...
            <div className="flex items-center gap-3 relative z-10">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-xs font-bold truncate text-theme">{user?.email}</span>
        ...
        <div className="space-y-1">
          {(!collapsed || isMobile) ? (
            <div className="flex items-center rounded-xl p-1 mb-1 bg-surface-2">
              <button onClick={() => setTheme('light')}
                className={cn(
                  ...
                  !isDark ? "bg-surface text-theme shadow-sm" : "text-theme-3 hover:text-theme-2"
                )}>
                <Sun size={13} /> Light
              </button>
              <button onClick={() => setTheme('dark')}
                className={cn(
                  "flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all",
                  isDark ? "bg-command-blue text-white shadow-sm" : "text-theme-3 hover:text-theme-2"
                )}>
                <Moon size={13} /> Dark
              </button>
            </div>
            ...
            <button onClick={cycleTheme}
              className="flex items-center gap-3 w-full p-3 rounded-xl transition-all group justify-center text-theme-2 hover:text-theme hover:bg-surface-2">
              {themeIcon}
            </button>
          )}
          {!isMobile && (
            <button onClick={() => setCollapsed(!collapsed)}
              className={cn(
                "flex items-center gap-3 w-full p-3 rounded-xl transition-all group text-theme-2 hover:text-theme hover:bg-surface-2",
  ...
  return (
    <>
      <div className="md:hidden fixed top-0 left-0 right-0 z-40 flex items-center justify-between px-4 py-3 border-b border-theme shadow-sm bg-surface">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-bold tracking-tight text-theme">{APP_NAME}</span>
        </div>
        <button onClick={() => setMobileOpen(true)} className="p-2 rounded-xl transition-all text-theme-2 hover:bg-surface-2">
          <Menu size={22} />
        </button>
      </div>

      <div className="hidden md:flex h-full">{sidebarContent(false)}</div>
        ...
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative flex h-full">
            ...
            <button onClick={() => setMobileOpen(false)}
              className="absolute top-4 right-[-48px] p-2 bg-surface rounded-xl shadow-lg text-theme-2">
              <X size={20} />
            </button>
          </div>
        </div>
